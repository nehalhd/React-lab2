...
export default function RegisterForm() {
  ...
  //Regular expression for white spaces
  const nameRegex = /^\b[A-Z][A-Z 0-9._%-]+\b$/i;
  ...
  const changeData = (e) => {
    ...
    //Set the new value of user name and set previous values for other data
      ...
      //Function to check for numbers in the bigining of username
      function namecheck(s) {
        return nameRegex.test(s);
      }
      //Variable holdes true or false (the return of namecheck function)
      let nCheck = namecheck(RegisterData.username);
